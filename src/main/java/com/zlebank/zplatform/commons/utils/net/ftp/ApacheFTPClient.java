package com.zlebank.zplatform.commons.utils.net.ftp;
...
public class ApacheFTPClient extends AbstractFTPClient {
	...
	@Override
	public void upload(String uploadPath, String fileName, File file)
			throws IOException {
		ftp = new FTPClient();
	...
	public void download(String downloadPath, String fileName,
			String targetDir, String targetName) throws IOException {
		ftp = new FTPClient();
		try {
			ftp.connect(getSeverIp(), getPort());
	...
	private void changeWorkingDirectory(String path) throws IOException {
		if (!path.startsWith(FILE_DESCRIPTOR)) {
			throw new RuntimeException(
					"uploadPath must start with character '/'");
		}
				...
				if (!ftp.changeWorkingDirectory(directyory)) {
					...
					if (!String.valueOf(reply).startsWith(FTP_SUCCESS_CODE_PRE)) {
						throw new IOException(
								"make directory error.upload path:" + path
										+ " failed.ftp replay code:" + reply);
					}
